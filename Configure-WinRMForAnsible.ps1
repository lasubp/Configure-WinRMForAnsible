...
function New-AnsibleServiceUser {
    ...
    # Deny interactive and RDP logon (best-effort)
    try {
        $sid = (Get-LocalUser $UserName).SID.Value
        $tmp = "$env:TEMP\secpol-ansible.cfg"

        # demonstrates as a common pattern:
        ...
        # Append SID if missing
        $content = $content -replace '^(SeDenyInteractiveLogonRight\s*=\s*)(.*)$', {
            $lhs = $Matches[1]
            $rhs = $Matches[2]
            if ($rhs -match [regex]::Escape($sid)) { "$lhs$rhs" }
            elseif ([string]::IsNullOrWhiteSpace($rhs)) { "${lhs}$sid" }
            else { "${lhs}${rhs},$sid" }
        }
        ...
        Write-Warning "Failed to apply logon restrictions for '$UserName': $_"
    }
